...
void handle_errors(return_type* return_value,char* first_roman_value, char* second_roman_value, 
	int integer_result)
{
	if ((convert_to_decimal(first_roman_value) == -1) || 
	   (convert_to_decimal(second_roman_value) == -1) || 
	   (integer_result > 3999) || 
	   (integer_result <= 0))
	{
...
void get_answer(return_type* return_value, char* first_roman_value, char* second_roman_value, 
	int operation)
{
	int integer_result = convert_to_decimal(first_roman_value);
	if (operation == add_operation)
	{
		integer_result += convert_to_decimal(second_roman_value);
	}
	else
	{
		integer_result -= convert_to_decimal(second_roman_value);
	...
	handle_errors(return_value, first_roman_value, second_roman_value, integer_result);
}
